...
# setup-subbridge-e2e-test-local clone and build the polkadot relay chain and khala parachain nodes
# then start the nodes locally and run the sygma setup scripts
setup-subbridge-e2e-test-local:
	./scripts/phala-subbridge/e2e_build_launch_local.sh
	./scripts/phala-subbridge/e2e_setup.sh

# setup-subbridge-e2e-test-remote is trying to use the prebuild relay chain node binary from polkadot
# only use this cmd when running in github CI
setup-subbridge-e2e-test-remote:
	./scripts/phala-subbridge/e2e_build_launch_remote.sh
	./scripts/phala-subbridge/e2e_setup.sh

# build-subbridge-e2e-test-image-local builds the e2e test docker image using the chain data generated by setup-subbridge-e2e-test-local
# this e2e image is a relay chain + phala parachain with sygma pallets simulation env
# run this cmd when building locally
build-subbridge-e2e-test-image-local:
	@echo "building the subbridge e2e test docker image..."
	@echo "dockerfile in use: Dockerfile_subbridge_e2e_local"
	docker build --file ./Dockerfile_subbridge_e2e_local -t sygma_substrate_pallets_subbridge_e2e_preconfigured .

# build-subbridge-e2e-test-image-remote builds the e2e test docker image using the chain data generated by setup-subbridge-e2e-test-remote
# this e2e image is a relay chain + phala parachain with sygma pallets simulation env
# run this cmd in Github CI
build-subbridge-e2e-test-image-remote:
	@echo "building the subbridge e2e test docker image..."
	@echo "dockerfile in use: Dockerfile_subbridge_e2e_remote"
	docker build --file ./Dockerfile_subbridge_e2e_remote -t sygma_substrate_pallets_subbridge_e2e_preconfigured .
...
# cleans the auto generated subbridge repo and log files
cleanup-subbridge:
	./scripts/phala-subbridge/cleanup.sh
